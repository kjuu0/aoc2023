...
use day4::{card_points_hashset_sum, card_points_linear_search_sum, scratchcards_sum};
...
pub fn pt2_benchmark(c: &mut Criterion) {
    c.bench_function("scratchcards_sum official input", |b| {
        b.iter_batched_ref(
            || BufReader::new(File::open("src/benches/input.txt").unwrap()),
            |reader| assert_eq!(scratchcards_sum(reader), 8172507),
            criterion::BatchSize::SmallInput,
        );
    });
}

criterion_group!(benches, pt1_hashset_benchmark, pt1_linear_benchmark, pt2_benchmark);
